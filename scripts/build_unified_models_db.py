...
def main() -> int:
    ...
    for filename, raw_entry in models.items():
        ...
        url_subfolder_category = None
        if "/text_encoders/" in url_lower:
            url_subfolder_category = "text_encoders"
        elif "/loras/" in url_lower:
            url_subfolder_category = "loras"
        elif "/vae/" in url_lower:
            url_subfolder_category = "vae"
        elif "/embeddings/" in url_lower:
            url_subfolder_category = "embeddings"
        elif "/latent_upscale_models/" in url_lower:
            url_subfolder_category = "latent_upscale_models"
        elif "/diffusion_models/" in url_lower:
            url_subfolder_category = "diffusion_models"

        category = url_subfolder_category or cloud_category or manager_category or manager_type_category or own_category
        category_verified = bool(url_subfolder_category or cloud_category or manager_category or manager_type_category)
        if not url_subfolder_category and existing_category_verified and existing_category in ALLOWED_EXPLORER_CATEGORIES:
        ...
        if category in {"checkpoints", "", "unknown"}:
            if "loras/" in url_lower or "loras" in directory_lower or "lora" in row_fn_lower:
                ...
                category = "vae"
                category_verified = True
            elif "latent_upscale_models/" in url_lower or "latent_upscale_models" in directory_lower or "upscaler" in row_fn_lower or "resizer" in row_fn_lower:
                category = "latent_upscale_models"
                if directory_lower in {"", "checkpoints"}:
                    entry["directory"] = "latent_upscale_models"
                category_verified = True
            elif "diffusion_models/" in url_lower or "diffusion_models" in directory_lower or "minimax_h3" in row_fn_lower or "minimaxh3" in row_fn_lower or "krea" in row_fn_lower or "convrot" in row_fn_lower or repo_lower.startswith("winnougan/") or repo_lower.startswith("comfy-org/"):
        ...
        if base_applicable:
            ...
            if existing_base_verified and existing_base and existing_base.lower() != "unknown":
                ...
                inferred_minimax = infer_minimax_base(repo_id_for_base, row_filename)
                ...
                if manager_base:
                    ...
                    elif inferred_minimax:
                        base_value = inferred_minimax
                        base_verified = source in {"comfyui_manager_model_list", "cloud_marketplace_export", "priority_repo_scrape"}
                        base_from = "inferred_minimax_family"
                    else:
                        if inferred_wan:
